...
import com.hybridavenger69.mtlasers.common.containers.customhandler.CardItemHandler;
...
import static com.hybridavenger69.mtlasers.util.MiscTools.tooltipMaker;
...
public class BaseCard extends Item {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level world, List<Component> tooltip, TooltipFlag flag) {
        ...
        if (!sneakPressed) {
            tooltip.add(Component.translatable("mtlasers.tooltip.item.show_settings")
                    .withStyle(ChatFormatting.GRAY));
        } else {
            String currentMode = getNamedTransferMode(stack).toString();
            MutableComponent toWrite = tooltipMaker("mtlasers.tooltip.item.card.mode", ChatFormatting.GRAY.getColor());
            ...
            toWrite.append(tooltipMaker("mtlasers.tooltip.item.card.mode." + currentMode, modeColor));
            tooltip.add(toWrite);

            toWrite = tooltipMaker("mtlasers.tooltip.item.card.channel", ChatFormatting.GRAY.getColor());
            ...
            if (sneakyMode != -1) {
                toWrite = tooltipMaker("mtlasers.tooltip.item.card.sneaky", ChatFormatting.GRAY.getColor());
                toWrite.append(tooltipMaker("mtlasers.tooltip.item.card.sneaky." + Direction.values()[sneakyMode].toString().toUpperCase(Locale.ROOT), ChatFormatting.DARK_GREEN.getColor()));
                tooltip.add(toWrite);
            }
        }
    }
